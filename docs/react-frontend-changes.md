...
- Added: local env guidance (`frontend/.env.local`, not committed)
...
- `frontend/.env.local`: sets `VITE_API_BASE_URL=http://localhost:8080` for local API calls (not committed).
...
- `frontend/src/api/debug.ts`: exposes `window.sentimentScribeApi` helpers for manual smoke calls during Step 1.
- `frontend/src/main.tsx`: registers the dev-only debug helpers on startup.

### How to verify (commands + manual checks)

- Run the frontend and call `window.sentimentScribeApi.smoke()` in devtools to hit `GET /api/entries`.
- Optional: `window.sentimentScribeApi.verifyPassword('<password>')` to hit `POST /api/auth/verify`.
...
- `frontend/src/state/auth.tsx`: stores `isUnlocked` and auth status, backed by `sessionStorage["sentimentscribe.isUnlocked"]`.
