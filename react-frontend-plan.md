# React Frontend Integration Plan (MoodVerse)
...
- **Spring Boot backend** (existing): `http://localhost:8080`
  ...
  - CORS is already configured via `moodverse.cors.allowed-origins` (default `http://localhost:3000`) in `backend/src/main/resources/application.yml`.
...
- Backend allows the frontend origin via `moodverse.cors.allowed-origins` (default `http://localhost:3000`).
...
- Configure production CORS on the backend to allow the deployed frontend origin(s):
  - set `MOODVERSE_CORS_ORIGIN` (or `moodverse.cors.allowed-origins`) to the frontend origin(s)
...
All endpoints below were found in `backend/src/main/java/com/moodverse/web/*Controller.java`.
...
These are the exact Java records used by the controllers under `backend/src/main/java/com/moodverse/web/dto/*`.
...
- ⚠️ Requires backend clarification: `"Created new password."` persistence across restarts (likely runtime-only unless `MOODVERSE_PASSWORD` is set externally).
...
- Route guarding (temporary, intentional):
  - after successful `POST /api/auth/verify`, set `auth.isUnlocked = true` and persist `sessionStorage["moodverse.isUnlocked"] = "true"`
...
- UI updates on success:
  ...
  - Set client auth state:
    ...
    - `sessionStorage["moodverse.isUnlocked"] = "true"`
...
- [ ] Implement auth gate: redirect to `/` if `!isUnlocked` (restore `isUnlocked` from `sessionStorage["moodverse.isUnlocked"]` on app start)
...
- [ ] On `200`, set auth state + persist `sessionStorage["moodverse.isUnlocked"]="true"` + navigate to `/home`
...
- [ ] Configure backend CORS for the deployed frontend origin(s) via `MOODVERSE_CORS_ORIGIN` (or `moodverse.cors.allowed-origins`)
